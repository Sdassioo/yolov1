...
class TestInterface(object):
    ...
    def main(self):
        ...
        """
        opts = self.opts
        img_list = os.listdir(opts.test_image_path)
        trans = transforms.Compose([
            # transforms.Resize((112, 112)),
            transforms.ToTensor(),
        ])
        model = torch.load(opts.weight_path)   #其实不是weight,是保存的模型
        if opts.use_GPU:
            model.to(opts.GPU_id)
        for img_name in img_list:
            img_path = os.path.join(opts.test_image_path, img_name)
            ...
            preds = preds.permute(1,2,0)   # 7,7,30
            bbox = labels2bbox(preds)     # 经过转换，得到
    ...
    def draw_bbox(self, img, bbox):
        ...
        for i in range(n):
            ...
            cv2.putText(img, cls_name, p1, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (37, 230, 0),1)
            cv2.putText(img, str(confidence), (p1[0],p1[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (37, 230, 0),1)
        cv2.imshow("bbox", img)
        cv2.waitKey(0)
